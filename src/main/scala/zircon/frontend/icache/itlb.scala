...
class ITLBReq(implicit p: Parameters) extends BaseZirconBundle {
  val addr = UInt(vaddrBits.W)
}

class ITLBResp(implicit p: Parameters) extends BaseZirconBundle {
  val miss = Bool()
  val ppn  = UInt(ppnBits.W)
}

class ITLBRefill(implicit p: Parameters) extends BaseZirconBundle {
  val addr  = UInt(vaddrBits.W)
  val level = UInt(lgPgLevels.W)
  val pte   = new PTE
}

class ITLBInvalidate(implicit p: Parameters) extends BaseZirconBundle {
  val valid = Bool()
  val ppn   = UInt(ppnBits.W)
}

class ITLBPTWReq(implicit p: Parameters) extends PTWReq
class ITLBPTWResp(implicit p: Parameters) extends PTWResp

class ITLBPTW(implicit p: Parameters) extends BaseZirconBundle {
  val req  = Decoupled(new ITLBPTWReq)
  val resp = Flipped(Decoupled(new ITLBPTWResp))
}

class ITLB(nSets: Int, nWays: Int)(implicit p: Parameters) extends BaseZirconModule {
  val io = IO(new Bundle() {
    ...
    val ptw = new ITLBPTW
    val invalidate = Output(Vec(nWays, new ITLBInvalidate))
  })
 ...
 */
  def vpnHit(vpn: UInt, valid: Bool, tag: ITLBTagBundle) = {
    var tagMatch = valid
  ...
  def tagHit(asid: UInt, prv: UInt, vpn: UInt, valid: Bool, tag: ITLBTagBundle) = {
  ...
  class ITLBTagBundle extends Bundle {
  ...
  class ITLBDataBundle extends Bundle {
    val r   = Bool()
    val w   = Bool()
    val x   = Bool()
    val u   = Bool()
  ...
  val read_tags = tag_array.read(req_idx, do_req).map(_.asTypeOf(new ITLBTagBundle))
  val read_datas = data_array.read(req_idx, do_req).map(_.asTypeOf(new ITLBDataBundle))
  ...
  val invalid_index = RegInit(UInt(log2Ceil(icacheParams.nTLBSets).W), 0.U)
  ...
  io.ptw.resp.ready     := !io.stall && (state === s_wait)
  ...
  val refill_tag  = Wire(new ITLBTagBundle)
  val refill_data = Wire(new ITLBDataBundle)
  ...
  //  1.  ITLB Require Valid and ITLB HIT.
  //  2.  ITLB Refill Valid.
