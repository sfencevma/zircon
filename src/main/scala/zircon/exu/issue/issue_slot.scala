...
class IssueSlotIO(implicit p: Parameters) extends BaseZirconBundle {
  val kill = Input(Bool())
...
class IssueSlot(implicit p: Parameters) extends BaseZirconModule
  with ScalarOpConstants
{
  ...
  when (io.kill) {
    uop_ld_track := 0.U
  } .elsewhen (io.stall(LSU_PORT)) {
    uop_ld_track := uop_ld_track
  } .elsewhen (io.req.valid) {
  ...
  //  Slot Valid
  val do_grant = io.replay_req.valid && io.replay_req.bits.grant
  ...
  } .elsewhen (do_grant && !issued) {
    valid := false.B
  }

  //  Slot Issued
  val do_replay = io.replay_req.valid && io.replay_req.bits.replay
  val need_replay = uop_ld_track(0) && do_replay
  ...
  when (io.kill) {
    ...
    secondary := io.replay_req.bits.secondary
  }
