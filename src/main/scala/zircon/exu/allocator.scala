...
class Allocator(plWidth: Int)(implicit p: Parameters) extends BaseZirconModule {
  val io = IO(new Bundle() {
    ...
    val rsv_dealloc = Input(Vec(numRsvEntries, Bool()))

    //  Rob
    val rob_head = Output(UInt(robIdBits.W))
    val rob_tail = Output(UInt(robIdBits.W))

    //  Ldq
    val ldq_head = Output(UInt(ldqIdBits.W))
    val ldq_tail = Output(UInt(ldqIdBits.W))

    //  Stq
    val stq_head = Output(UInt(stqIdBits.W))
    val stq_tail = Output(UInt(stqIdBits.W))
  ...
  rob_freelist.io.flush               := io.kill
  rob_freelist.io.stall               := ldq_freelist.io.empty || stq_freelist.io.empty || rsv_freelist.io.empty
  ...
  io.rob_head := rob_freelist.io.head
  io.rob_tail := rob_freelist.io.tail

  //  Load-Queue Free List
  ldq_freelist.io.flush               := io.kill
  ldq_freelist.io.stall               := rob_freelist.io.empty || stq_freelist.io.empty || rsv_freelist.io.empty
  ...
  io.ldq_head := ldq_freelist.io.head
  io.ldq_tail := ldq_freelist.io.tail

  //  Store-Queue Free List
  stq_freelist.io.flush               := io.kill
  stq_freelist.io.stall               := rob_freelist.io.empty || ldq_freelist.io.empty || rsv_freelist.io.empty
  ...
  io.stq_head := stq_freelist.io.head
  io.stq_tail := stq_freelist.io.tail

  //  Reservation Station Free List
  rsv_freelist.io.kill          := io.kill
  rsv_freelist.io.stall         := rob_freelist.io.empty || ldq_freelist.io.empty || stq_freelist.io.empty
