...
class Satp(implicit p: Parameters) extends BaseZirconBundle {
  val mode = UInt(4.W)
...
class CustomCSRsIO(implicit p: Parameters) extends BaseZirconBundle {
  ...
  val read_port = Input(new CsrReadPortIO)
  val write_port = Flipped(Valid(new CsrWritePortIO))

  //  Interrupts and Exceptions
  val intrs = Flipped(Decoupled(new Interrupts))
  val xpt = Flipped(Valid(new Exceptions))

  //  Instruction Retire
  val ret = Input(Vec(retireWidth, Bool()))

  //  Flush when Modify CSRs
  val flush = Output(UInt(vaddrBits.W))
  ...
  //  PMU Update Info
  val pmu_upd = Input(new PMUUpdate)
...
class CustomCSRs(implicit p: Parameters) extends BaseZirconModule {
  ...
  val reg_marchid = WireInit(UInt(xLen.W))
  reg_marchid := 0.U
  ...
  val reg_mideleg = RegInit(UInt(xLen.W))
  ...
  mip.seip := reg_mip.seip | io.intrs.bits.seip
  ...
  val m_decoded_addr = m_mapping map { case (k, v) => k -> (io.write_port.bits.addr === k.U) }

  //  Mstatus
  when (io.write_port.valid) {
    val wdata = io.write_port.bits.data
    ...
    when (m_decoded_addr(CSRs.mie)) { reg_mie := wdata & support_interrupts }
  ...
  reg_mip.meip := io.intrs.bits.meip
  reg_mip.mtip := io.intrs.bits.mtip
  reg_mip.msip := io.intrs.bits.msip
  io.prv := reg_prv
  ...
  val reset_satp = Wire(new Satp)
  reset_satp := 0.U.asTypeOf(new Satp)
  val reg_satp = RegInit(new Satp, reset_satp)
  ...
  val s_decoded_addr = s_mapping map { case (k, v) => k -> (io.write_port.bits.addr === k.U) }
  when (io.write_port.valid) {
    val wdata = WireInit(io.write_port.bits.data)
    ...
    when (s_decoded_addr(CSRs.sie)) { reg_mie := (read_mie & ~read_mideleg) | (wdata & read_mideleg) }
    when (s_decoded_addr(CSRs.satp)) {
      val new_satp = wdata.asTypeOf(new Satp)
      reg_satp.mode := 9.U
  ...
  val f_decoded_addr = f_mapping map { case (k, v) => k -> (io.write_port.bits.addr === k.U) }
  when (io.write_port.valid) {
    val wdata = io.write_port.bits.data
  ...
  val mm_decoded_addr = mm_mapping map { case (k, v) => k -> (io.write_port.bits.addr === k.U) }
  ...
  when (io.write_port.valid) {
    ...
    for (n <- 0 until nPerfCounters) {
      when(mm_decoded_addr(CSRs.mhpmevent3 + n)) {
        val which_evt_sel = reg_mhpmevents(n)
        val mask = (!which_evt_sel(xLen - 1, hpmEvtBits).orR) && (wdata(hpmEvtBits - 1, 0) <= nPerfCounters.U)
        reg_mhpmevents(n) := Cat(Fill(xLen - hpmEvtBits, 0.U), wdata(hpmEvtBits - 1, 0) & Fill(hpmEvtBits, mask))
      }
    }
    for (n <- 0 until nPerfCounters) {
      when (mm_decoded_addr(CSRs.mhpmcounter3 + n)) { reg_mhpmcounters(n) := wdata }
    }
  }
  ...
  io.read_port := Mux1H(for ((k, v) <- read_mapping) yield read_decoded_addr(k) -> v)
  ...
  val exception = (io.xpt.valid && (ecall || ebreak || xpt)) || io.intrs.valid
  ...
  val tvec = Mux(ecall | ebreak, 0x800.U, Mux(delegate, read_stvec, read_mtvec))
  ...
  //  Update HPMC
  val support_mhpevts = VecInit(Seq (
    false.B,
    io.pmu_upd.mispred,
    exception,
    intr_vld,
    ...
    io.pmu_upd.is_mdu,
    io.pmu_upd.is_fpu,
    io.pmu_upd.icache_access,
    io.pmu_upd.icache_miss,
    io.pmu_upd.itlb_access,
    io.pmu_upd.itlb_miss,
    io.pmu_upd.dcache_read_access,
    io.pmu_upd.dcache_write_access,
    io.pmu_upd.dcache_atom_access,
    io.pmu_upd.dcache_read_miss,
    io.pmu_upd.dcache_write_miss,
    io.pmu_upd.dcache_atom_miss,
    io.pmu_upd.dtlb_access,
    io.pmu_upd.dtlb_miss,
    io.pmu_upd.stall,
    io.pmu_upd.flush,
    ecall,
    ebreak
  ) ++ Seq.fill(xLen-25) { false.B })
  ...
  for (n <- 0 until nPerfCounters) {
    ...
    when (hpmevt_ena && support_mhpevts(which_event)) {
      reg_mhpmcounters(which_event) := reg_mhpmcounters(which_event) + 1.U
    }
  }
  ...
  io.intrs.ready := RegNext(!(ecall | ebreak | mret | sret | xpt) & intr_vld)
