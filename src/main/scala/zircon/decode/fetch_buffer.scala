...
class FetchBuffer(plWidth: Int, numEntries: Int)(implicit p: Parameters) extends BaseZirconModule with ScalarOpConstants {
  val io = IO(new Bundle() {
    ...
    val sync_rl_ret = Input(Bool())
    val sync_aq_ret = Input(Bool())
  ...
  //  Enqueue
  val do_enq = !io.forward_stall && io.req.valid
  ...
  io.forward_stall := nums < enq_count
  ...
  switch (state_reg) {
    ...
    is (s_wait) {
      when (io.sync_rl_ret | io.sync_aq_ret) {
        state := s_normal
      }
    }
  }
  ...
  when (io.flush) {
    state_reg := s_normal
  } .elsewhen (io.sync_aq_ret | io.sync_rl_ret) {
    state_reg := state
  } .elsewhen (io.stall) {
    state_reg := state_reg
  } .otherwise {
  ...
  when (io.flush) {
    ...
    resp.valid := inst_nums > 0.U && (state_reg === s_normal)
  }
