...
class RenameStageReq(implicit p: Parameters) extends BaseZirconBundle {
  val valid     = Bool()
...
class Rename(plWidth: Int, numEntries: Int, float: Boolean)(implicit p: Parameters) extends BaseZirconModule with ScalarOpConstants {
  val io = IO(new Bundle(){
    ...
    val reqs = Flipped(Valid(Vec(plWidth, new RenameStageReq)))
...
class RenameStage(plWidth: Int)(implicit p: Parameters) extends BaseZirconModule with ScalarOpConstants {
  ...
  //
  rename_stage.io.flush := io.flush
  rename_stage.io.stall := io.stall
  fp_rename_stage.io.flush := io.flush
  fp_rename_stage.io.stall := io.stall

  //  Require
  val reqs = Wire(Vec(plWidth, new RenameStageReq))
  val fp_reqs = Wire(Vec(plWidth, new RenameStageReq))
  ...
  for (w <- 0 until plWidth) {
    ...
    reqs(w).ldst_vld  := io.reqs.bits(w).uop.ldst_vld && (io.reqs.bits(w).uop.ldst_type === RT_INT)
  ...
  for (w <- 0 until plWidth) {
    when (!io.stall) {
      ...
      resps.bits(w).lrs1_map := Mux(io.reqs.bits(w).uop.lrs1_type === RT_INT,
        rename_stage.io.resps(w).lrs1_map, fp_rename_stage.io.resps(w).lrs1_map)
      resps.bits(w).lrs2_map := Mux(io.reqs.bits(w).uop.lrs2_type === RT_INT,
        rename_stage.io.resps(w).lrs2_map, fp_rename_stage.io.resps(w).lrs2_map)
      resps.bits(w).lrs3_map := Mux(io.reqs.bits(w).uop.lrs3_type === RT_INT,
  ...
  for (w <- 0 until plWidth) {
    commit_reqs(w).valid := io.commit_reqs.bits(w).valid && (io.commit_reqs.bits(w).ldst_type === RT_INT)
