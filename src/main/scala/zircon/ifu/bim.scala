...
class BIM(nSets: Int)(implicit p: Parameters) extends BaseZirconModule {
  ...
  val upd_data = Mux(io.upd.bits.taken, Mux(io.upd.bits.old_bim === 3.U, io.upd.bits.old_bim, io.upd.bits.old_bim + 1.U),
    Mux(io.upd.bits.old_bim === 0.U, io.upd.bits.old_bim, io.upd.bits.old_bim - 1.U))
  ...
  io.resp.bim := Mux(do_bypass, bypass_data, bim.read(req_idx))
}