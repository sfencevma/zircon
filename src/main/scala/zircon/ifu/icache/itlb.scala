...
class ITLBPTWReq(implicit p: Parameters) extends BaseZirconBundle {
  val addr = UInt(vaddrBits.W)
}

class ITLBPTWResp(implicit p: Parameters) extends BaseZirconBundle {
  val pte   = new PTE
  val level = UInt(lgPgLevels.W)
...
class ITLBPTW(implicit p: Parameters) extends BaseZirconBundle {
  ...
  val resp = Flipped(Valid(new ITLBPTWResp))
}
...
class ITLB(nSets: Int, nWays: Int)(implicit p: Parameters) extends BaseZirconModule {
  ...
  val read_tags = tag_array.read(req_idx).map(_.asTypeOf(new ITLBTagBundle))
  val read_datas = tag_array.read(req_idx).map(_.asTypeOf(new ITLBDataBundle))
